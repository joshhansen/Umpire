...
fn criterion_benchmark(c: &mut Criterion) {
    for src in [Location::new(0, 0), Location::new(4, 4)].iter() {
        for dims in [Dims::new(10, 10), Dims::new(11, 11)].iter() {
            for (wrap_name,wrapping) in [("both",Wrap2d::BOTH), ("horiz",Wrap2d::HORIZ), ("vert",Wrap2d::VERT), ("neither",Wrap2d::NEITHER)].iter() {
                c.bench_function(
                    format!("nearest_adjacent_unobserved {} src={} {}", dims, src, wrap_name).as_ref(),
                    |b| b.iter(|| 
