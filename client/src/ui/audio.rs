...
impl Noisy for UnitType {
    fn freqs(&self) -> Vec<f32> {
        match self {
            ...
            UnitType::Transport => vec![
                LetterOctave(Letter::C, 2).hz(),
                LetterOctave(Letter::G, 2).hz(),
            ],
            UnitType::Destroyer => vec![LetterOctave(Letter::D, 2).hz()],
            UnitType::Submarine => vec![LetterOctave(Letter::C, 2).hz()],
            UnitType::Cruiser => vec![
                LetterOctave(Letter::A, 1).hz(),
                LetterOctave(Letter::C, 1).hz(),
            ],
            UnitType::Battleship => vec![
                LetterOctave(Letter::C, 1).hz(),
                LetterOctave(Letter::Db, 1).hz(),
            ],
    ...
    match sound {
        ...
        Sounds::Unit(unit_type) => {
            ...
            if !freqs.is_empty() {
                ...
                let oscillator =
                    Oscillator::new(oscillator::waveform::Square, amp_env, freq_env, ());
    ...
    event_loop.run(move |_id, result| {
        ...
        match data {
            cpal::StreamData::Output {
                buffer: cpal::UnknownTypeOutputBuffer::U16(ref mut buffer),
            } => {
                let buffer: &mut [u16] = &mut *buffer;

                let buffer: &mut [[u16; CHANNELS as usize]] =
