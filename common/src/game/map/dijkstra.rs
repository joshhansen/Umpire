...
impl ShortestPaths {
    ...
    pub fn direction_of(&self, dims: Dims, wrapping: Wrap2d, dest: Location) -> Option<Direction> {
        self.shortest_path(dest).map(|path| {
            ...
            let inc: Vec2d<i32> = wrapping
                .wrapped_sub(dims, self.start_loc, first_loc)
                .unwrap();
            let dir = Direction::try_from(inc).expect(
                format!(
                    "{} not a Direction; first_loc: {:?}, start_loc: {:?}",
                    inc, first_loc, self.start_loc
                )
                .as_str(),
            );
            dir
...
pub static RELATIVE_NEIGHBORS: [Vec2d<i32>; 8] = [
    REL_NEIGHB_BOTTOM_LEFT,
    REL_NEIGHB_BOTTOM,
    REL_NEIGHB_BOTTOM_RIGHT,
    REL_NEIGHB_LEFT,
    REL_NEIGHB_RIGHT,
    REL_NEIGHB_TOP_LEFT,
    REL_NEIGHB_TOP,
    REL_NEIGHB_TOP_RIGHT,
];
...
fn all_resolved_neighbors_iter<'a, T: 'a, S>(
    tiles: &'a S,
    loc: Location,
    wrapping: Wrap2d,
) -> impl Iterator<Item = (Location, &'a T)>
where
    S: Source<T>,
{
    RELATIVE_NEIGHBORS
...
pub fn neighbors_iter<'a, T, F, N, S>(
    tiles: &'a S,
    loc: Location,
    rel_neighbs: N,
    filter: &'a F,
...
pub fn neighbors_terrain_only<T: Source<Tile>>(
    tiles: &T,
    loc: Location,
    unit_type: UnitType,
    wrapping: Wrap2d,
...
/// such).
pub fn neighbors_unit_could_move_to<T: Source<Tile>>(
    tiles: &T,
    unit: &Unit,
    wrapping: Wrap2d,
) -> HashSet<Location> {
...
pub fn neighbors_unit_could_move_to_iter<'a, T: Source<Tile>>(
    tiles: &'a T,
    unit: &'a Unit,
    wrapping: Wrap2d,
) -> impl Iterator<Item = Location> + 'a {
...
pub fn shortest_paths<T, F: Filter<T>, S: Source<T>>(
    tiles: &S,
    source: Location,
    filter: &F,
    wrapping: Wrap2d,
...
mod test {
    ...
    use crate::{
        game::{
            map::{terrain::Terrain, LocationGrid, LocationGridI, Tile},
            obs::Obs,
    ...
    use super::{
        nearest_adjacent_unobserved_reachable_without_attacking, neighbors, neighbors_terrain_only,
        shortest_paths, All, Filter, PacifistXenophileUnitMovementFilter, Source,
        UnitMovementFilter, Xenophile, RELATIVE_NEIGHBORS,
    };

    fn neighbors_all_unit<T: Source<Tile>>(
        tiles: &T,
        loc: Location,
        unit: &Unit,
        wrapping: Wrap2d,
    ...
    fn test_neighbors_terrain_only() {
        ...
        let neighbs_neither =
            neighbors_terrain_only(&map, loc, UnitType::Infantry, Wrap2d::NEITHER);
    ...
    *xx\n\
    ...
    xxx",
        ...
        let shortest_neither =
            shortest_paths(&map, loc, &filter, Wrap2d::NEITHER, armor.moves_remaining());
        ...
        let shortest_horiz =
            shortest_paths(&map, loc, &filter, Wrap2d::HORIZ, armor.moves_remaining());
        ...
        let shortest_vert =
            shortest_paths(&map, loc, &filter, Wrap2d::VERT, armor.moves_remaining());
        ...
        let shortest_both =
            shortest_paths(&map, loc, &filter, Wrap2d::BOTH, armor.moves_remaining());
...
...",
    ...
    fn _test_nearest_adjacent_unobserved_reachable_without_attacking(dims: Dims) {
        ...
        for wrapping in [Wrap2d::BOTH, Wrap2d::HORIZ, Wrap2d::VERT, Wrap2d::NEITHER].iter() {
            ...
            let naurwa = nearest_adjacent_unobserved_reachable_without_attacking(
                &grid, src, &unit, *wrapping,
            );
    ...
    pub fn test_unit_movement_filter() {
        ...
        let u1 = Unit::new(UnitID::new(0), l1, UnitType::Infantry, a, "u1");

        let u2 = Unit::new(UnitID::new(1), l2, UnitType::Infantry, a, "u2");
