...
impl Wrap {
    ...
    fn wrapped_sub(self, dimension_width: u16, coord0: u16, coord1: u16) -> Result<i32, WrapError> {
        if coord0 >= dimension_width {
            return Err(WrapError::OutOfBounds {
                coord0: true,
                coord: coord0,
                dimension_width,
            });
...
impl Wrap2d {
    pub fn values() -> [Self; 4] {
        [Self::NEITHER, Self::HORIZ, Self::VERT, Self::BOTH]
    }
    ...
    pub fn wrapped_add(self, dims: Dims, loc: Location, inc: Vec2d<i32>) -> Option<Location> {
        ...
        self.vert
            .wrapped_add(dims.height, loc.y, inc.y)
            .map(|new_y| Location { x: new_x, y: new_y })
    }
    ...
    pub fn wrapped_sub(
        ...
        loc0: Location,
        loc1: Location,
    ) -> Result<Vec2d<i32>, Wrap2dError> {
        let inc_x =
            self.horiz
                .wrapped_sub(dims.width, loc0.x, loc1.x)
                .map_err(|err| match err {
                    WrapError::OutOfBounds { coord0: true, .. } => {
                        Wrap2dError::OutOfBounds { loc: loc0, dims }
                    }
                    WrapError::OutOfBounds { coord0: false, .. } => {
                        Wrap2dError::OutOfBounds { loc: loc1, dims }
                    }
                })?;

        self.vert
            .wrapped_sub(dims.height, loc0.y, loc1.y)
...
mod test {
    ...
    use super::{Dims, Location, Vec2d, Wrap, Wrap2d};
    ...
    fn test_wrapped_add() {
        ...
        */
        ...
        for (i, rel_neighb) in RELATIVE_NEIGHBORS.iter().enumerate() {
            assert_eq!(
                Wrap2d::BOTH.wrapped_add(dims, loc, *rel_neighb),
                results_both[i]
            );
            assert_eq!(
                Wrap2d::HORIZ.wrapped_add(dims, loc, *rel_neighb),
                results_horiz[i]
            );
            assert_eq!(
                Wrap2d::VERT.wrapped_add(dims, loc, *rel_neighb),
                results_vert[i]
            );
            assert_eq!(
                Wrap2d::NEITHER.wrapped_add(dims, loc, *rel_neighb),
                results_neither[i]
            );
        }

        assert_eq!(
            Wrap2d::HORIZ.wrapped_add(
                Dims {
                    width: 5,
                    height: 1
                },
        ...
        assert_eq!(
            ...
            Some(Location { x: 2, y: 0 })
        );

        assert_eq!(
            Wrap2d::NEITHER.wrapped_add(
                Dims {
                    width: 5,
                    height: 1
                },
    ...
    fn test_wrapped_sub_1d() {
        ...
        let coord1 = 4;
        assert_eq!(
            Wrap::Wrapping.wrapped_sub(dimension_width, coord0, coord1),
            Ok(-2)
        );
        assert_eq!(
            Wrap::NonWrapping.wrapped_sub(dimension_width, coord0, coord1),
            Ok(4)
        );
        assert_eq!(
            Wrap::Wrapping.wrapped_sub(dimension_width, coord1, coord0),
            Ok(2)
        );
        assert_eq!(
            Wrap::NonWrapping.wrapped_sub(dimension_width, coord1, coord0),
            Ok(-4)
        );
    ...
    fn test_wrapped_sub_2d() {
        ...
        */
        ...
        assert_eq!(
            Wrap2d::BOTH.wrapped_sub(dims, loc0, loc1),
            Ok(Vec2d::new(-2, -2))
        );
        assert_eq!(
            Wrap2d::HORIZ.wrapped_sub(dims, loc0, loc1),
            Ok(Vec2d::new(-2, 4))
        );
        assert_eq!(
            Wrap2d::VERT.wrapped_sub(dims, loc0, loc1),
            Ok(Vec2d::new(3, -2))
        );
        assert_eq!(
            Wrap2d::NEITHER.wrapped_sub(dims, loc0, loc1),
            Ok(Vec2d::new(3, 4))
        );

        assert_eq!(
            Wrap2d::BOTH.wrapped_sub(dims, loc1, loc0),
            Ok(Vec2d::new(2, 2))
        );
        assert_eq!(
            Wrap2d::HORIZ.wrapped_sub(dims, loc1, loc0),
            Ok(Vec2d::new(2, -4))
        );
        assert_eq!(
            Wrap2d::VERT.wrapped_sub(dims, loc1, loc0),
            Ok(Vec2d::new(-3, 2))
        );
        assert_eq!(
            Wrap2d::NEITHER.wrapped_sub(dims, loc1, loc0),
            Ok(Vec2d::new(-3, -4))
        );
